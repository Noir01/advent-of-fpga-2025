...
module Harness = Cyclesim_harness.Make (Day06_solver.I) (Day06_solver.O)
...
let expected_p1 = 4277556L
let expected_p2 = 3263827L
...
let run_solver ~width ~height rows =
  ...
  Harness.run_advanced
    ...
    ~create:Day06_solver.hierarchical
    testbench
;;

let%expect_test "Sample test - grid encoding and solver completion" =
  let width, height, rows = Day06_parser.parse_string sample_input in
  print_s [%message "Grid dimensions" (width : int) (height : int)];
  run_solver ~width ~height rows;
  [%expect
    {|
    ("Grid dimensions" (width 15) (height 4))
    (Results (part1 4277556) (part2 3263827) (!cycles_run 167))
    |}]
;;
