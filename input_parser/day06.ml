(** Day 06 input parser.
    ...
    Parses a grid where each cell can be a digit 0-9, space, plus, or asterisk. Encodes
    each cell as a 4-bit value:
    - 0-9: digits
    - 10: space ' '
    - 11: '+' operator
    - 12: '*' operator

    Returns (width, height, rows) where each row is a list of encoded 4-bit values. *)

open! Core
open! Util

(** Encode a single character to its 4-bit value *)
let encode_char c =
  match c with
...
let parse_string ?(verbose = false) raw =
  ...
  let rows =
    List.map lines ~f:(fun line ->
      let padding = List.init (width - String.length line) ~f:(fun _ -> 10) in
