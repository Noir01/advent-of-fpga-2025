(** Day 04 input parser.
    ...
    Parses a grid of '.' and '@' characters, adds a border of '.' around all sides
    and encodes each row as a bit vector ('.' = 0, '@' = 1)
    Returns (width, height, row_strings) where width/height include the border *)

open! Core
open! Util

(** Parse input and add border. Returns (width, height, rows) where each row
    is a string of '0' and '1' characters (for later encoding to bits).
    Width and height include the 1-cell border on all sides. *)
